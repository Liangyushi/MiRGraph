...
class RGCNpre(torch.nn.Module):
    ...
    def decoderMLP(self,xm,xg,edge):
        ...
        #print(s.size())
        return s
    def forward(self,data,label_edge):
        xm,xg=self.encoder(data)
...
def trainEpoch(model,trainloader,scaler,lossF,device):
    ...
    for step,dat in enumerate(trainloader):
        ...
        dat=dat.to(device)
        #print(dat)
        out = model(dat,dat['regulate'].edge_label_index)
        out=out.view(-1)
        #print(out.size())
...
if __name__ == '__main__':
    ...
    for epoch in range(1, 1001):
        ...
        if val_aupr > best_val_aupr:
            ...
            torch.save(model, './RGCN/RGCNbest_gpu_directDot.pt')
            time_elapsed = time.time() - since
    ...
    with open('./RGCN/RGCNResult_gpu_directDot.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
        pickle.dump([trainloss, trainauc,trainaupr,valloss,valauc,valaupr,lrchange], f)

    torch.save(model, './RGCN/RGCN_gpu_directDot.pt')