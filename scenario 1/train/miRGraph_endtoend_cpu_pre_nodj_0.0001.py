...
if __name__ == '__main__':
    ...
    pretrained_file='./results/hnuResults/TransCNN.pth'
    model = transfer_model(pretrained_file, model)

    pretrained_file='./results/hnuResults/HGT_linkloader.pth'
    model = transfer_model(pretrained_file, model)
    ...
    for epoch in range(1, 1001):
        ...
        if val_aupr > best_val_aupr:
            ...
            torch.save(model, './miRGraph_endtoend/Modelbest_cpu_pre_nodj_0.0001.pt')
            time_elapsed = time.time() - since
    ...
    with open('./miRGraph_endtoend/ModelResult_cpu_pre_nodj_0.0001.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
        pickle.dump([trainloss, trainauc,trainaupr,valloss,valauc,valaupr,lrchange], f)

    torch.save(model, './miRGraph_endtoend/Model_cpu_pre_nodj_0.0001.pt')