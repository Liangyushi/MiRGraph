...
def trainEpoch(model,trainloader,scaler,lossF,device):
    ...
    for step,dat in enumerate(trainloader):
        ...
        #print(num_nodes)
        out = model(dat.x_dict,dat.edge_index_dict,dat['gene'].seq,dat['regulate'].edge_label_index)
        out=out.view(-1)
        #print(out.size())
        rel=dat['regulate'].edge_label
        #print(rel.size())
...
if __name__ == '__main__':
    ...
    for epoch in range(1, 1001):
        ...
        if val_aupr > best_val_aupr:
            ...
            torch.save(model, './HGT_BiLSTM/HGT_BiLSTMbest_gpu_mlp.pt')
            time_elapsed = time.time() - since
    ...
    with open('./HGT_BiLSTM/HGT_BiLSTMResult_gpu_mlp.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
        pickle.dump([trainloss, trainauc,trainaupr,valloss,valauc,valaupr,lrchange], f)

    torch.save(model, './HGT_BiLSTM/HGT_BiLSTM_gpu_mlp.pt')